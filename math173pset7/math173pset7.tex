...
\begin{enumerate}[(a)]
    ...
    \hop Now, let's check the Laplacian. Let $f(r \theta) = r^{|n|}e^{in\theta}$. For $|n| \ge 2$, we have $f_{rr} = r^{|n| - 2}(|n|-1)|n|e^{in\theta}$ and $f_{\theta\theta} = -r^{|n|}|n|^2e^{in\theta}$ and $f_r = |n|r^{|n|-1}e^{in\theta}$. Thus, $f$ is harmonic. So, $P_r$ is the sum of harmonic functions, so it is harmonic.
\end{enumerate}
